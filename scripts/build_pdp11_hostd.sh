#!/bin/bash
# build_pdp11_hostd.sh - cross-compile pdp11-hostd in the Dockerized PetaLinux
# container (only the pdp11-hostd recipe - fast, sstate is warm) and copy the
# resulting ARM binary to deploy/pdp11-hostd.
#
...
#
# Output: deploy/pdp11-hostd (ARM ELF for the board).
...
RECIPE_SRC="$REPO/petalinux/meta-user/recipes-apps/pdp11-hostd"
DEPLOY="$REPO/deploy"
...
RECIPE_DST="$PROJ_HOST/project-spec/meta-user/recipes-apps/pdp11-hostd"

[ -f "$RECIPE_SRC/pdp11-hostd.bb" ] || { echo "ERROR: missing recipe $RECIPE_SRC"; exit 1; }
...
echo "== building pdp11-hostd recipe in container =="
...
docker run --rm \
  ...
  "$IMG" bash -lc "
    ...
    cd $PROJ
    petalinux-build -c pdp11-hostd -x do_install
    BIN=\$(find build/tmp/work -path '*pdp11-hostd*/image/usr/bin/pdp11-hostd' | head -1)
    [ -n \"\$BIN\" ] || { echo 'ERROR: built binary not found'; exit 1; }
    cp -f \$BIN /deploy/pdp11-hostd
    echo \"BUILD_OK -> /deploy/pdp11-hostd\"
  "

echo "== verifying =="
file "$DEPLOY/pdp11-hostd"
ls -la "$DEPLOY/pdp11-hostd"
