...
- `tu58fs` and `picocom` recipes, plus `bsp.cfg` enabling
  `CONFIG_SERIAL_UARTLITE` (the `kl1`-`kl3` consoles) and `CONFIG_TUN` (the
...
The guest has a working Ethernet interface: 2.11BSD sees a DEUNA as `de0` and
routes over the LAN at 0% packet loss. Controlled by `have_xu_net` in
`zynq_top.vhd`; setting it to `0` and rebuilding removes networking without
any other wiring change.

### Implementation

`xu.vhd` (from the upstream pdp2011 core) already contained a complete DEUNA
implementation that had never been enabled: a second, embedded PDP-11 core
(`cpu0`/`mmu0`, with its own console and line clock on a private local
unibus) executing pre-assembled DEUNA microcode. That microcode implements
PCSR0-3, GETPCBB/GETCMD/PDMD and the descriptor rings.

Upstream, that microcode drives a physical ESP32 over bit-banged SPI. This
project replaces only the SPI half: `core/xuaxi.vhd` presents the microcode's
existing XF/RT/RL/SRDY register contract unchanged, and bridges it over
AXI-Lite to `pdp11-hostd` on the PS, which moves frames to and from a Linux
`tap0` joined to `br0` alongside `eth0`.

Consequently the repository contains no DEUNA protocol or descriptor-ring
logic — only a byte-buffer shuttle and the ESP32 wire framing.
`docs/xu-networking-plan.md` documents the register map, wire framing,
diagnostic registers, the two defects found during bring-up, and the
hypotheses that were tested and refuted.

### Configuration

`pdp11-hostd` creates `tap0`, joins it to `br0` with `eth0`, and moves the
board's DHCP lease to the bridge, making the guest a directly addressable
host on the LAN. On the guest:
...
On the PS side, `tcpdump -i tap0` or `-i br0` shows guest traffic.

### Expected behaviour

**Round-trip times are reported as exactly 16.667 ms.** This is the guest's
60 Hz line-clock period, not transit time; the reply returns on the wire in
approximately 90 µs. 2.11BSD measures RTT in whole clock ticks and defers
received-packet processing to software-interrupt level, which runs in step
with the same tick. Measured from Linux at a ping interval that is not a
whole number of ticks, the actual figure is ~7 ms, predominantly the PDP-11
executing its own IP stack.

**Broadcast traffic is filtered.** A real DEUNA accepts all broadcast; on a
contemporary LAN that fills the guest's six-entry receive ring with ARP for
other hosts, mDNS and SSDP. `pdp11-hostd` accepts unicast addressed to the
guest, and broadcast only for ARP targeting the guest's own address, which it
learns by inspecting the guest's outbound traffic. No configuration is
required. Accept and drop counts are reported by the `/status` endpoint.
