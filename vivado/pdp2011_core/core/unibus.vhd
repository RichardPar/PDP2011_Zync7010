...
-- debug: unibus device-register bus (I/O page), for the serial monitor
      dbg_dev_addr : out std_logic_vector(17 downto 0);              -- device register address
      dbg_dev_dati : out std_logic_vector(15 downto 0);              -- data read from a device
      dbg_dev_dato : out std_logic_vector(15 downto 0);              -- data written to a device
      dbg_dev_rd   : out std_logic;                                  -- device read strobe
      dbg_dev_wr   : out std_logic;                                  -- device write strobe

-- debug: RL11 DMA bus-master (disk->memory transfers), for the serial monitor
      dbg_dma_addr : out std_logic_vector(17 downto 0);              -- DMA address (RL11 disk transfer target)
      dbg_dma_dato : out std_logic_vector(15 downto 0);               -- DMA data being written to memory
      dbg_dma_wr   : out std_logic;                                   -- DMA write strobe

-- debug: RH11/RP06 DMA bus-master (disk->memory transfers), write-strobe
-- only - lets the front panel color-code disk activity by controller (RL02
-- vs RP06) without needing the address/data too. Sourced from whichever of
-- rh0's two busmaster interfaces is actually active (see have_rh70 below).
      dbg_dma_rh_wr : out std_logic;                                  -- RH11 DMA write strobe
...
-- rl controller
      ...
      -- RL disk AXI-Lite backend (sddisk.vhd) + interrupt, routed to the PS
      rl_disk_s_axi_aclk    : in  std_logic := '0';
      rl_disk_s_axi_aresetn : in  std_logic := '1';
      rl_disk_s_axi_awaddr  : in  std_logic_vector(11 downto 0) := (others => '0');
      rl_disk_s_axi_awvalid : in  std_logic := '0';
      rl_disk_s_axi_awready : out std_logic;
      rl_disk_s_axi_wdata   : in  std_logic_vector(31 downto 0) := (others => '0');
      rl_disk_s_axi_wstrb   : in  std_logic_vector(3 downto 0) := (others => '0');
      rl_disk_s_axi_wvalid  : in  std_logic := '0';
      rl_disk_s_axi_wready  : out std_logic;
      rl_disk_s_axi_bresp   : out std_logic_vector(1 downto 0);
      rl_disk_s_axi_bvalid  : out std_logic;
      rl_disk_s_axi_bready  : in  std_logic := '0';
      rl_disk_s_axi_araddr  : in  std_logic_vector(11 downto 0) := (others => '0');
      rl_disk_s_axi_arvalid : in  std_logic := '0';
      rl_disk_s_axi_arready : out std_logic;
      rl_disk_s_axi_rdata   : out std_logic_vector(31 downto 0);
      rl_disk_s_axi_rresp   : out std_logic_vector(1 downto 0);
      rl_disk_s_axi_rvalid  : out std_logic;
      rl_disk_s_axi_rready  : in  std_logic := '0';
      rl_disk_irq           : out std_logic;
...
-- rh controller
      ...
      -- RH disk (RP06) AXI-Lite backend (sddisk.vhd) + interrupt, routed to
      -- the PS - same bridge pattern as rl_disk_s_axi_* above
      rh_disk_s_axi_aclk    : in  std_logic := '0';
      rh_disk_s_axi_aresetn : in  std_logic := '1';
      rh_disk_s_axi_awaddr  : in  std_logic_vector(11 downto 0) := (others => '0');
      rh_disk_s_axi_awvalid : in  std_logic := '0';
      rh_disk_s_axi_awready : out std_logic;
      rh_disk_s_axi_wdata   : in  std_logic_vector(31 downto 0) := (others => '0');
      rh_disk_s_axi_wstrb   : in  std_logic_vector(3 downto 0) := (others => '0');
      rh_disk_s_axi_wvalid  : in  std_logic := '0';
      rh_disk_s_axi_wready  : out std_logic;
      rh_disk_s_axi_bresp   : out std_logic_vector(1 downto 0);
      rh_disk_s_axi_bvalid  : out std_logic;
      rh_disk_s_axi_bready  : in  std_logic := '0';
      rh_disk_s_axi_araddr  : in  std_logic_vector(11 downto 0) := (others => '0');
      rh_disk_s_axi_arvalid : in  std_logic := '0';
      ...
      rh_disk_s_axi_rready  : in  std_logic := '0';
...
--    );
  ...
  kw0: kw11l port map(
   ...
   rl0: rl11 port map(
      ...
      disk_s_axi_aclk    => rl_disk_s_axi_aclk,
      disk_s_axi_aresetn => rl_disk_s_axi_aresetn,
      disk_s_axi_awaddr  => rl_disk_s_axi_awaddr,
      disk_s_axi_awvalid => rl_disk_s_axi_awvalid,
      disk_s_axi_awready => rl_disk_s_axi_awready,
      disk_s_axi_wdata   => rl_disk_s_axi_wdata,
      disk_s_axi_wstrb   => rl_disk_s_axi_wstrb,
      disk_s_axi_wvalid  => rl_disk_s_axi_wvalid,
      disk_s_axi_wready  => rl_disk_s_axi_wready,
      disk_s_axi_bresp   => rl_disk_s_axi_bresp,
      disk_s_axi_bvalid  => rl_disk_s_axi_bvalid,
      disk_s_axi_bready  => rl_disk_s_axi_bready,
      disk_s_axi_araddr  => rl_disk_s_axi_araddr,
      disk_s_axi_arvalid => rl_disk_s_axi_arvalid,
      disk_s_axi_arready => rl_disk_s_axi_arready,
      disk_s_axi_rdata   => rl_disk_s_axi_rdata,
      disk_s_axi_rresp   => rl_disk_s_axi_rresp,
      disk_s_axi_rvalid  => rl_disk_s_axi_rvalid,
      disk_s_axi_rready  => rl_disk_s_axi_rready,
      disk_irq           => rl_disk_irq,
   ...
   rh0: rh11 port map(
      ...
      disk_s_axi_aclk    => rh_disk_s_axi_aclk,
      disk_s_axi_aresetn => rh_disk_s_axi_aresetn,
      disk_s_axi_awaddr  => rh_disk_s_axi_awaddr,
      disk_s_axi_awvalid => rh_disk_s_axi_awvalid,
      disk_s_axi_awready => rh_disk_s_axi_awready,
      disk_s_axi_wdata   => rh_disk_s_axi_wdata,
      disk_s_axi_wstrb   => rh_disk_s_axi_wstrb,
      disk_s_axi_wvalid  => rh_disk_s_axi_wvalid,
      disk_s_axi_wready  => rh_disk_s_axi_wready,
      disk_s_axi_bresp   => rh_disk_s_axi_bresp,
      disk_s_axi_bvalid  => rh_disk_s_axi_bvalid,
      disk_s_axi_bready  => rh_disk_s_axi_bready,
      disk_s_axi_araddr  => rh_disk_s_axi_araddr,
      disk_s_axi_arvalid => rh_disk_s_axi_arvalid,
      disk_s_axi_arready => rh_disk_s_axi_arready,
      disk_s_axi_rdata   => rh_disk_s_axi_rdata,
      disk_s_axi_rresp   => rh_disk_s_axi_rresp,
      disk_s_axi_rvalid  => rh_disk_s_axi_rvalid,
      disk_s_axi_rready  => rh_disk_s_axi_rready,
      disk_irq           => rh_disk_irq,
...
-- expose the unibus device-register bus for the serial debug monitor
    dbg_dev_addr <= unibus_addr;
    dbg_dev_dati <= unibus_dati;
    dbg_dev_dato <= unibus_dato;
    dbg_dev_rd   <= unibus_control_dati;
    dbg_dev_wr   <= unibus_control_dato;

    -- expose the RL11 DMA bus-master (disk->memory) for the debug monitor
    dbg_dma_addr <= rl0_addr;
    dbg_dma_dato <= rl0_dato;
    dbg_dma_wr   <= rl0_control_dato;

    -- expose the RH11/RP06 DMA bus-master write strobe (front panel only -
    -- rh0_control_dato is the 18-bit busmaster path (have_rh70=0), the RH70
    -- one is the 22-bit path used when have_rh70=1 (modelcode=70, the
    -- current setting - see zynq_top.vhd); only one is ever actually driven
    -- for a given have_rh70, so selecting is safe rather than OR-ing them.
    dbg_dma_rh_wr <= rh0_control_dato when have_rh70 = 0 else rh70_bus_master_control_dato;
